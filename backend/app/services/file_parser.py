...
def detect_file_format(df: pd.DataFrame) -> str:
    ...
    Returns:
        형식 이름 ('toss_bank', 'mirae', 'kb', 'standard', 'unknown')
    """
    if df.empty:
        return 'unknown'
...
def transform_kb_to_standard(df: pd.DataFrame) -> pd.DataFrame:
    """
    KB증권 형식을 표준 형식으로 변환
...
def transform_to_standard(df: pd.DataFrame, file_format: str) -> pd.DataFrame:
    ...
    Args:
        df: 원본 DataFrame
        file_format: 파일 형식 ('toss_bank', 'mirae', 'kb', 'standard')
    ...
    if file_format == 'toss_bank':
        ...
        return transform_kb_to_standard(df)
